/**
 * Deploy the Confidential Salary Benchmarking Pool contract to a Midnight
 * network (undeployed local devnet by default; use --network preview|preprod).
 *
 * The deployed instance starts empty; role/location categories are opened via
 * `npm run cli` → open category, then contributors submit salaries privately.
 *
 * Non-interactive: npm run setup runs straight through.
 */
...
import { witnesses, createSalaryPoolPrivateState } from './witnesses';
...
/** Retry a submission through a flaky RPC/relay gateway. */
function isTransientSubmissionError(err: unknown): boolean {
  ...
  return (
    msg.includes('disconnected from') ||
...
async function submitWithRetry<T>(
  ...
  for (let attempt = 1; attempt <= maxAttempts; attempt++) {
    try {
      ...
      if (!isTransientSubmissionError(err) || attempt === maxAttempts) throw err;
      console.log(
        `  ⏳ ${label} interrupted by a transient RPC disconnect (attempt ${attempt}/${maxAttempts}); retrying in ${delayMs / 1000}s...`,
      );
      await new Promise((r) => setTimeout(r, delayMs));
    }
  }
...
async function main() {
  console.log('\n╔══════════════════════════════════════════════════════════════╗');
  console.log('║  Deploy Confidential Salary Benchmarking Pool                  ║');
  console.log('╚══════════════════════════════════════════════════════════════╝\n');
  ...
  for (let attempt = 1; attempt <= MAX_RETRIES; attempt++) {
    try {
      ...
      const isTransient = isTransientSubmissionError(err);

      if (!isDustShortage && !isTransient) {
      ...
      if (
        ...
        (fullError.includes('Failed to connect to Proof Server') ||
          fullError.includes('connect ECONNREFUSED 127.0.0.1:6305'))
      ) {
      ...
      if (isTransient) {
        if (attempt < MAX_RETRIES) {
          console.log(`  ⏳ Transient RPC disconnect during deploy (attempt ${attempt}/${MAX_RETRIES}); retrying in ${RETRY_DELAY_MS / 1000}s...`);
